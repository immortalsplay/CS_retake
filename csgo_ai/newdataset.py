...
class SmallCSGODataset(Dataset):
    ...
    def __len__(self):
        # every has 1100 - frames_per_sample + 1
        return min(self.num_samples * (1100 - self.frames_per_sample + 1), len(self.data))

    def __getitem__(self, idx):
        data_idx = idx // (1100 // self.frames_per_sample)
        frame_idx = idx % (1100 // self.frames_per_sample)

        if frame_idx < self.frames_per_sample:
            data = np.concatenate(
                [np.repeat(self.data[data_idx][0][np.newaxis, :], self.frames_per_sample - frame_idx - 1, axis=0),
                 self.data[data_idx][0:frame_idx + 1]], axis=0)
            label = self.label[data_idx][frame_idx]
        else:
            data = self.data[data_idx][frame_idx - self.frames_per_sample + 1:frame_idx + 1]
            label = self.label[data_idx][frame_idx]

        data = data.reshape(-1, 125, 200, 3)  # reshape
        data = np.array([cv2.resize(frame, self.new_size) for frame in data])
        data = np.transpose(data, (0, 3, 1, 2))  # transpose
